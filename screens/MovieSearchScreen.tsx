    ...
    <View style={styles.container}>
      ...
      <View style={styles.customHeader}>
        <View style={styles.searchContainer}>
          <Image source={SearchIcon} style={[styles.iconStyles, { transform: [{ scaleX: -1 }] }]} />
          <TextInput
          ...
          />
            <TouchableOpacity onPress={clearSearch}>
              <Image source={CrossIcon} style={styles.iconStyles} />
            </TouchableOpacity>
        </View>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const displayText = query.length > 2 ? item.title : genreMap[item.genre_ids[0]] || 'Unknown'; // Show genre name or movie title
        ...
        ListFooterComponent={
          loading && moviesToDisplay.length > 0 ? <ActivityIndicator size="small" color="#000" style={styles.loadingMore} /> : null
        }
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 20, color: '#fff', textAlign: 'center', paddingVertical: 5, fontWeight:'700' },
